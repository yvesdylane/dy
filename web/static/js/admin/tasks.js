(function () {
  ...
  function renderTasks(tasks) {
    ...
    tasks.forEach(function (t) {
      ...
      var fileTag = t.file_name
        ? '<span class="inline-flex items-center gap-1 text-xs text-zinc-500 max-w-full overflow-hidden"><svg class="w-3.5 h-3.5 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg><span class="truncate">' + esc(t.file_name) + '</span></span>'
        : "";
      var urlTag = t.supporting_doc
        ? '<span class="inline-flex items-center gap-1 text-xs text-teal-600 dark:text-teal-400"><svg class="w-3.5 h-3.5 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>URL</span>'
        : "";
      ...
      html += '<div class="task-card p-3 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 cursor-pointer hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors break-words overflow-hidden" data-id="' + t.id + '">'
  ...
  function showEditModal(task) {
    var m = document.getElementById("modalOverlay");
    var c = document.getElementById("modalContent");
    var deadlineVal = task.submission_deadline ? task.submission_deadline.slice(0, 16) : "";
    c.innerHTML = ''
      + '<h3 class="text-lg font-bold mb-1">Edit Task</h3>'
      + '<p class="text-xs text-zinc-500 mb-4">Update the task details below.</p>'
      + '<form id="taskForm" class="space-y-3" onsubmit="return false">'
      + '<label class="block text-sm font-medium text-zinc-700 dark:text-zinc-300">Name *'
      + '<input name="name" type="text" value="' + esc(task.name) + '" required class="mt-1 w-full px-3 py-2 rounded-lg border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/50"></label>'
      + '<label class="block text-sm font-medium text-zinc-700 dark:text-zinc-300">Description *'
      + '<textarea name="description" rows="3" required class="mt-1 w-full px-3 py-2 rounded-lg border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/50">' + esc(task.description) + '</textarea></label>'
      + '<div class="grid grid-cols-2 gap-3">'
      + '<label class="block text-sm font-medium text-zinc-700 dark:text-zinc-300">Department *'
      + '<select name="department" required class="mt-1 w-full px-3 py-2 rounded-lg border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/50">'
      + '<option value="ISM"' + (task.department === "ISM" ? " selected" : "") + '>ISM</option><option value="SWE"' + (task.department === "SWE" ? " selected" : "") + '>SWE</option><option value="CGWD"' + (task.department === "CGWD" ? " selected" : "") + '>CGWD</option><option value="EDM"' + (task.department === "EDM" ? " selected" : "") + '>EDM</option><option value="CSN"' + (task.department === "CSN" ? " selected" : "") + '>CSN</option><option value="DBMS"' + (task.department === "DBMS" ? " selected" : "") + '>DBMS</option><option value="NWS"' + (task.department === "NWS" ? " selected" : "") + '>NWS</option></select></label>'
      + '<label class="block text-sm font-medium text-zinc-700 dark:text-zinc-300">Total Marks *'
      + '<input name="total_mark_on" type="number" value="' + task.total_mark_on + '" min="1" required class="mt-1 w-full px-3 py-2 rounded-lg border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/50"></label>'
      + '</div>'
      + '<label class="block text-sm font-medium text-zinc-700 dark:text-zinc-300">Submission Deadline *'
      + '<input name="submission_deadline" type="datetime-local" value="' + deadlineVal + '" required class="mt-1 w-full px-3 py-2 rounded-lg border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/50"></label>'
      + '<label class="block text-sm font-medium text-zinc-700 dark:text-zinc-300">Supporting Doc URL'
      + '<input name="supporting_doc" type="url" value="' + esc(task.supporting_doc || "") + '" placeholder="https://" class="mt-1 w-full px-3 py-2 rounded-lg border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/50"></label>'
      + '<label class="block text-sm font-medium text-zinc-700 dark:text-zinc-300">Attach File' + (task.file_name ? ' <span class="text-xs text-zinc-400">(current: ' + esc(task.file_name) + ')</span>' : '')
      + '<input name="file" type="file" class="mt-1 w-full text-sm text-zinc-500 file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-medium file:bg-teal-50 dark:file:bg-teal-950/30 file:text-teal-700 dark:file:text-teal-300 hover:file:bg-teal-100 dark:hover:file:bg-teal-950/50"></label>'
      + '<p id="taskFormErr" class="text-red-500 text-xs hidden"></p>'
      + '<div class="flex gap-2 pt-2">'
      + '<button type="button" onclick="closeModal()" class="flex-1 py-2.5 rounded-lg border border-zinc-300 dark:border-zinc-700 text-sm font-medium hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors">Cancel</button>'
      + '<button type="submit" id="taskSubmitBtn" class="flex-1 py-2.5 rounded-lg bg-teal-600 hover:bg-teal-700 text-white text-sm font-medium transition-colors">Save Changes</button></div></form>';
    m.classList.remove("hidden");
    ...
    document.getElementById("taskForm").addEventListener("submit", function (e) {
      ...
      formData.append("total_mark_on", f.elements["total_mark_on"].value);
      var deadline = f.elements["submission_deadline"].value;
      if (deadline) formData.append("submission_deadline", new Date(deadline).toISOString());
      var urlVal = f.elements["supporting_doc"].value.trim();
      if (urlVal) formData.append("supporting_doc", urlVal);
      var fileInput = f.elements["file"];
      if (fileInput && fileInput.files[0]) {
      ...
      fetch("/api/admin/tasks/" + task.id, {
        ...
        .then(function (res) {
          ...
          if (res.ok) {
            ...
            var err = document.getElementById("taskFormErr");
            err.textContent = res.detail || "Error updating task";
            err.classList.remove("hidden");
          }
        })
        .catch(function () {
